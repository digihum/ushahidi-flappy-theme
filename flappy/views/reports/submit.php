...
<?php if ($site_submit_report_message != ''): ?>
			...
			<div class="big-block">
				...
				<p><?PHP 
					$block = new extra_form_parts();
					$block->block("form_intro");
					$block->block("form_required_statement");
				?></p>
				<?php if ($form_error): ?>
				<!-- red-box -->
				<div class="red-box">
				...
				<div class="report_row">
					...
					<?php
							...
							$html = category::form_tree('incident_category', $selected_categories, 2);
							...
							foreach ($result as $node) {
								$node->lastChild->nodeValue = ucfirst($node->lastChild->nodeValue);
							}
							$query = "//li"; 
							$xpath = new DOMXPath($this->domDocument); 
							$result = $xpath->query($query); 
							$categories = array();
							foreach ($result as $node) {
								$categories[strtolower($node->getAttribute("title"))] = $node;
							...
							while(count($categories) >= ($total / 2)){
								$columns[0]->appendChild(array_shift($categories));
							}
							while($node = array_shift($categories)){
				...
				<div class="report_row">
					...
					<p><?PHP
						$block->block("form_location");
					?></p>
					<p><?PHP
						$block->block("form_city_select");
					?></p>
					<?php print form::dropdown('select_city',$cities,'', ' class="select" '); ?>
				</div>
				<?php endif; ?>
				<p><?PHP
						$block->block("form_map_select");
				?></p>
				<div class="report_row half-width">
					...
					<div class="report-find-location">
						<?PHP
							$block->block("form_map_guide");
						?>
						<div id="panel" class="olControlEditingToolbar"></div>
						<?PHP
							$block->block("form_delete");
						?>
						<div class="btns">
							<ul>
				...
				<div id="divNews" class="report_row">
					...
					<?PHP
						$block->block("form_news");
						$block->block("form_optional");
					?>
