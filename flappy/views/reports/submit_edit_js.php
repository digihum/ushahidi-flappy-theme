...
?>
		...
		jQuery(window).load(function() {
			...
			console.log(<?php echo $longitude; ?>, <?php echo $latitude; ?>);
		...
		function refreshFeatures(event) {
			...
			for(i=0; i < vlayer.features.length; i++) {
				...
				if (vlayer.features.length == 1 && vlayer.features[i].geometry.CLASS_NAME == "OpenLayers.Geometry.Point") {
					...
					var lonlat = OpenLayers.Projection.transform(vlayer.features[i].geometry, 'EPSG:3857', 'EPSG:4326');
					
					var jqxhr = $.ajax( "https://maps.googleapis.com/maps/api/geocode/json?latlng=" + lonlat.y + "," + lonlat.x )
					  .done(function(response) {
						new_location = new Array();
						for(x in response['results'][0]['address_components']){
							if(response['results'][0]['address_components'][x]['types'][0]!="country"){
								if(new_location.indexOf(response['results'][0]['address_components'][x]['long_name']) == -1){
									new_location.push(response['results'][0]['address_components'][x]['long_name']);
								}
							}
						}
						first = new_location.shift();
						new_location[0] = first + " " + new_location[0];
						place = new_location.join(",");
						$("#location_name").val(place);
					  });
